...
def create_invwarp_all2best(name="invwarp_all2best",
	...
	invwarpwf.connect([
		(inputnode, datainnode, [('data_dir','base_directory'),
								('best_id','best_id'),
								('all_ids','other_id')]),
		(datainnode, renamenode, [('best_id','best_id'),
								('other_id','other_id')]),
		(datainnode, invwarpnode, [('other2best','warp'),
								('other','reference')]),
		(renamenode, invwarpnode, [('out_file','inverse_warp')]),
		(invwarpnode, dataoutnode, [('inverse_warp','container.scan.@invwarp'
